...
func InitUser() {
	...
	c, err := userservice.NewClient(
		...
		client.WithClientBasicInfo(&rpcinfo.EndpointBasicInfo{ServiceName: constant.FeedServiceName}),
	)
	if err != nil {
		panic(err)
	}
...
func GetUserInfo(ctx context.Context, userId int64, info operation.TVideoInfo) (userInfo *user.UserInfo, err error) {
	var rpcReq = &user.UserInfoRequest{
		UserId:  userId,
		QueryId: info.AuthorId,
	}
